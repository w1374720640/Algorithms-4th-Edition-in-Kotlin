 ...
 * 数组规模N的起始值为1000，排序后打印N、估计排序用时、实际排序用时以及在N增倍之后两次用时的比例。
 ...
 */
fun doublingTest(maxSize: Int, sortMethod: (Array<Double>) -> Unit, O: (N: Int) -> Double) {
    val createArray: (Int) -> Array<Double> = { getDoubleArray(it, ArrayInitialState.RANDOM)}
    doublingTest(maxSize, sortMethod, createArray, O)
}

fun doublingTest(maxSize: Int, sortMethod: (Array<Double>) -> Unit, createArray: (Int) -> Array<Double>, O: (N: Int) -> Double) {
    var N = 1000
    var preTime = 0L
    while (N <= maxSize) {
...
fun main() {
    ...
    doublingTest(10_0000, ::selectionSort) { N -> N.toDouble().pow(2) }
    println("insertionSort:")
    doublingTest(10_0000, ::insertionSort) { N -> N.toDouble().pow(2) }

    //希尔排序不同增长序列有不同的性能，参考 https://blog.csdn.net/Foliciatarier/article/details/53891144
    println("shellSort:")
    doublingTest(500_0000, ::shellSort) { N -> N.toDouble().pow(4.0 / 3) }
}
