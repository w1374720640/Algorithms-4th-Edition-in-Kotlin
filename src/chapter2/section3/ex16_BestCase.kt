 ...
 */
fun ex16_BestCase(size: Int): Array<Double> {
    val array = Array(size) { it.toDouble() }
    ex16_BestCase(array, 0, array.size - 1)
    return array
}

fun ex16_BestCase(array: Array<Double>, start: Int, end: Int) {
    if (start >= end) return
    val mid = (start + end) / 2
    ex16_BestCase(array, start, mid - 1)
    ex16_BestCase(array, mid + 1, end)
...
private fun compareWithRandomArray() {
    val size = 100_0000
    val bestArray = ex16_BestCase(size)
