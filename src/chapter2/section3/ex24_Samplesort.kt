 ...
 */
fun <T : Comparable<T>> ex24_Samplesort(array: Array<T>, k: Int) {
    ...
    ex24_Samplesort(array, 0, sampleSize / 2, array.size - 1)
}

fun <T : Comparable<T>> ex24_Samplesort(array: Array<T>, start: Int, halfSampleSize: Int, end: Int) {
    //数组已经有序
    ...
    when {
        ...
        midIndex == start + halfSampleSize -> {
            ...
            ex24_Samplesort(array, midIndex + 1, newHalfSampleSize, end)
        }
        //右半边有序，只排序左半边
        midIndex == end - halfSampleSize -> {
            ...
            ex24_Samplesort(array, start, newHalfSampleSize, midIndex - 1)
        }
        //正常逻辑
        else -> {
            repeat(newHalfSampleSize) {
...
fun main() {
    cornerCases { ex24_Samplesort(it, 3) }

    val sortMethods: Array<Pair<String, (Array<Double>) -> Unit>> = arrayOf(
            "Quick Sort" to ::quickSort,
            "ex24" to { array: Array<Double> -> ex24_Samplesort(array, 10) }
    )
