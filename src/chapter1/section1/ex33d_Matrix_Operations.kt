...
//矩阵和向量之积
//长度为n的向量可以表示为n行1列的矩阵
fun ex33d_Matrix_Operations(a: Array<Array<Int>>, x: Array<Int>): Array<Array<Int>> {
    val matrix = Array(x.size) { arrayOf(0) }
    for (i in x.indices) {
        matrix[i][0] = x[i]
    }
    return ex33b_Matrix_Operations(a, matrix)
}
...
fun main() {
    ...
    println()
    val result = ex33d_Matrix_Operations(a, b)
