 ...
 * 返回层数和摔碎的鸡蛋数（不是查找次数）
 */
fun ex24a(array: IntArray, start: Int = 0, end: Int = array.size - 1, count: Int = 0): Pair<Int, Int> {
    ...
    return if (array[mid] == 0) {
        ex24a(array, mid + 1, end, count)
    } else {
        ex24a(array, start, mid - 1, count + 1)
    }
}
 ...
 * 所以，总成本为1+1/2*lg(F/2)=1+1/2*(lgF-lg2)=1+1/2*lgF-1/2*1=(1+lgF)/2
 */
fun ex24b(array: IntArray): Pair<Int, Int> {
    ...
    do {
        ...
        if (end >= array.size - 1) {
            return ex24a(array, start = end / 2, end = array.size - 1)
        }
        if (array[end] == 1) {
            return ex24a(array, start = end / 2, end = end, count = 1)
        }
    } while (true)
}
