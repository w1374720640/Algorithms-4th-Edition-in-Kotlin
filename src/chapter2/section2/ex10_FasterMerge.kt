 ...
 */
fun <T : Comparable<T>> ex10_FasterMerge(array: Array<T>) {
    val extraArray = array.copyOf()
    ex10_FasterMerge(array, extraArray, 0, array.size - 1)
}

fun <T : Comparable<T>> ex10_FasterMerge(array: Array<T>, extraArray: Array<T>, start: Int, end: Int) {
    if (start >= end) return
    val mid = (start + end) / 2
    ex10_FasterMerge(array, extraArray, start, mid)
    ex10_FasterMerge(array, extraArray, mid + 1, end)
